...
class Home extends Component {
  ...
  componentDidMount() {
   ...
   .then(artworkData => {
     const artworks = artworkData.map(apiData => {
       return Object.assign({}, apiData, {favorited: false})
     })
  ...
  favoriteArtwork = (event, artworkId) => {
   console.log(event.target)
   this.state.artworks.map(artworkObj => {
      ...
      <div className="Home">
        ...
        <div className="row">
          ...
          <div className='col-9'>
            ...
            showModal={this.showModal} favoriteArtwork={this.favoriteArtwork}
            />
          </div>
        </div>
        <center>
